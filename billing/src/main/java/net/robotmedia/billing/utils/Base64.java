...
public class Base64 {
	...
	public static byte[] decode(byte[] source, int off, int len, byte[] decodabet)
			...
			if (sbiDecode >= WHITE_SPACE_ENC) { // White space Equals sign or better
				if (sbiDecode >= EQUALS_SIGN_ENC) {
					...
					if (sbiCrop == EQUALS_SIGN) {
						...
						} else if ((b4Pos == 3 && bytesLeft > 2) || (b4Pos == 4 && bytesLeft > 1)) {
					...
					b4[b4Pos++] = sbiCrop;
					if (b4Pos == 4) {
						outBuffPos += decode4to3(b4, 0, outBuff, outBuffPos, decodabet);
						b4Pos = 0;
					}
				}
			} else {
		...
		// b4Pos != 0.  There can be at most 2 equal signs at the end of
		...
		if (b4Pos != 0) {
			...
			b4[b4Pos++] = EQUALS_SIGN;
			outBuffPos += decode4to3(b4, 0, outBuff, outBuffPos, decodabet);
		}

		byte[] out = new byte[outBuffPos];
